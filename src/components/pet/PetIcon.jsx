    ...
    return (
        <div>
            <svg xmlns="http://www.w3.org/2000/svg" 
                viewBox="0 0 600 600" 
                className={`w-14 h-14 z-10 absolute text-[#7F9FB5] dark:text-[#3a4a5b] ${position} transform -translate-y-1/2 transition-transform duration-300 ease-in-out md:-rotate-90`}
            >
                <path
...
const PetIcon = ({ name, image, isSelected, onClick }) => {
    ...
    return (
        <div onClick={onClick} className="w-40 h-40 flex flex-col items-center group relative cursor-pointer">
            ...
            <h2 className="text-center text-xl font-['IBM Plex Sans'] text-slate-700 dark:text-zinc-200 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20">{name}</h2>
            <button 
                className="w-32 h-32 rounded-full overflow-hidden mt-3 shadow-md shadow-gray-700 dark:shadow-gray-500 flex items-center justify-center p-0 border-0 z-10"
                onClick={(e) => {
                    e.stopPropagation();
                    onClick();
                }}
